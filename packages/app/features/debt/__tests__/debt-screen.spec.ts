...
const test = mergeTests(localTest, currenciesPickerTest, dateInputTest);
...
test("On load", async ({
	...
	await expect(page.getByTestId("debt-sync-status")).not.toBeAttached();
});
...
test("Header shows sync status and receipt link when applicable", async ({
	...
	await expect(page.getByTestId("debt-sync-status")).toBeVisible();
...
test.describe("Amount", () => {
	...
	test("'debts.update' mutation", async ({
		...
		// Optimistic cache update commits the new amount right away, so the
		// save button (shown only while the field is dirty) unmounts before
		// the mutation actually resolves.
		await expect(saveAmountButton).not.toBeAttached();
...
test.describe("Remove", () => {
	...
	test("asks for confirmation, then handles error and success", async ({
		...
		await expect(dialog).toContainText("This will remove debt forever");
		...
		api.mockFirst("debts.getAllUser", []);
		api.mockFirst("debts.getByUserPaged", { items: [], count: 0, cursor: 0 });
		const buttonWithLoader = withLoader(removeDebtButton);
		await removeDebtButton.click();
		await dialog.getByRole("button", { name: "Yes" }).click();

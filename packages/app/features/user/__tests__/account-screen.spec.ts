...
test.describe("Avatar", () => {
	test.describe("editor open", () => {
		...
		test("user can upload an avatar", async ({
			...
			api.mockFirst("user.changeAvatar", { url: faker.image.url() });
			...
			await snapshotQueries(async () => {
				...
				await awaitCacheKey("user.changeAvatar");
	...
	test("user can remove an avatar", async ({
		...
		api.mockFirst("user.changeAvatar", undefined);
...
test.describe("Password change", () => {
	...
	test("'user.changePassword' mutation", async ({
		api,
		prevPasswordField,
		passwordField,
		passwordRetypeField,
		...
		api.mockFirst("user.changePassword", () => {
			throw new TRPCError({
				code: "UNAUTHORIZED",
				message: `Mock "user.changePassword" error`,
			});
		});
		...
		await snapshotQueries(async () => {
			...
			await awaitCacheKey("user.changePassword", { error: 1 });
			await verifyToastTexts(
				`Error changing password: Mock "user.changePassword" error`,
			);
		});

		// Form retains values after error — no need to re-fill
		const pause = api.createPause();
		api.mockFirst("user.changePassword", async () => {
			await pause.promise;
		});
		...
		await snapshotQueries(
			async () => {
				...
				await awaitCacheKey("user.changePassword", { pending: 1 });
			},
			{ name: "loading" },
		);
		...
		await snapshotQueries(
			async () => {
				pause.resolve();
				await awaitCacheKey("user.changePassword");
				await expect(buttonWithLoader).toBeHidden();
			},
...
test("'user.logout' mutation", async ({
	page,
	api,
	mockBase,
	logoutButton,
	...
	await mockBase();
	api.mockFirst("user.logout", () => {
		throw new TRPCError({
			code: "INTERNAL_SERVER_ERROR",
			message: `Mock "user.logout" error`,
		});
	});
	await page.navigate({ to: "/user" });
	...
	await snapshotQueries(async () => {
		...
		await awaitCacheKey("user.logout", { error: 1 });
		await verifyToastTexts(`Logout failed: Mock "user.logout" error`);
	});
	await page.expectUrl({ to: "/user" });

	api.mockUtils.noAuthPage();
	const pause = api.createPause();
	api.mockFirst("user.logout", async () => {
		await pause.promise;
	});
	...
	await snapshotQueries(
		async () => {
			...
			await expect(buttonWithLoader).toBeVisible();
			await awaitCacheKey("user.logout", { pending: 1 });
		},
		{ name: "loading" },
	);
	...
	await snapshotQueries(
		async () => {
			pause.resolve();
			await awaitCacheKey("user.logout");

...
const renameIndexes = async (db: Database, direction: "up" | "down") => {
	for (const newName of indexes) {
		const oldName = newName
			...
			.replace("User", "Account");
		const [from, to] =
			direction === "down" ? [newName, oldName] : [oldName, newName];
		await sql`ALTER INDEX ${sql.id(from)} RENAME TO ${sql.id(to)}`.execute(db);
	}
};

const renameTriggers = async (db: Database, direction: "up" | "down") => {
	const triggers = [
		[
...
export const up = async (db: Database) => {
	await renameIndexes(db, "up");
	await renameTriggers(db, "up");
	await renameUserSchema(db);
};

export const down = async (db: Database) => {
	await renameTriggers(db, "down");
	await restoreAccountSchema(db);
	await renameIndexes(db, "down");
};

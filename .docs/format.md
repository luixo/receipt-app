...
Run `yarn format` to apply. Run `yarn format:prepare` after adding a new workspace or a new generated-file pattern.
...
- Re-run `yarn format:prepare` whenever a new workspace is added so the new package's ignored paths are included.

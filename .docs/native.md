...
- CSS variables for colors are generated from `@heroui/theme` semantic colors via `generate-colors.ts`. Run `bun run --filter '@ra/mobile' css:prepare` after changing theme colors.

...
describe("receiptItemPayers.add", () => {
	describe("input verification", () => {
		...
		describe("user", () => {
			...
			test("is already added to the item", async ({ ctx }) => {
				...
				await insertReceiptItemPayer(ctx, receiptItemId, participantUserId);
				...
				await expectTRPCError(
					...
					"CONFLICT",
					`User "${participantUserId}" already pays for item "${receiptItemId}".`,
				);
			});
		});
	...
	describe("functionality", () => {
		test("multiple participants are added to multiple items in multiple receipts", async ({
			...
			await insertReceiptItemPayer(ctx, anotherReceiptItemId, user.id);
